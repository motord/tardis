...
from playhouse.postgres_ext import PostgresqlExtDatabase, JSONField, HStoreField, UUIDField
import momoko
#import logging
...
class PostgresqlAsyncDatabase(PostgresqlExtDatabase):
        ...
        if recursive:
            for query, fk in reversed(list(self.dependencies(delete_nullable))):
                if fk.null and not delete_nullable:
                    yield gen.Task(fk.model_class.update(**{fk.name: None}).where(query).execute)
                else:
                    yield gen.Task(fk.model_class.delete().where(query).execute)
        yield gen.Task(self.delete().where(self._meta.primary_key == self.get_id()).execute)
